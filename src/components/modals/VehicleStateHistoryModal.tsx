...
import VehicleStateHistoryService, {
  VehicleStateHistoryRecord,
} from '../../services/vehicle-state-history.service';
import { useNotify } from '../../services/notify';
import { useAuth } from '../../hooks/useAuth';

interface VehicleStateHistoryModalProps {
  open: boolean;
  onClose: () => void;
  vehicleId: number;
  vehiclePlate: string;
...
export default function VehicleStateHistoryModal({
  open,
  onClose,
  vehicleId,
  vehiclePlate,
}: VehicleStateHistoryModalProps) {
  const [history, setHistory] = useState<VehicleStateHistoryRecord[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [recordToDelete, setRecordToDelete] = useState<VehicleStateHistoryRecord | null>(null);
  const [deleting, setDeleting] = useState(false);
  const [editDialogOpen, setEditDialogOpen] = useState(false);
  const [recordToEdit, setRecordToEdit] = useState<VehicleStateHistoryRecord | null>(null);
  ...
  const fetchHistory = async () => {
    ...
    setError(null);
    try {
      const data = await VehicleStateHistoryService.getVehicleStateHistory(vehicleId);
      setHistory(data);
    } catch (err: any) {
  ...
  const handleDeleteClick = (record: VehicleStateHistoryRecord) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true);
    try {
      await VehicleStateHistoryService.deleteVehicleStateHistoryRecord(recordToDelete.id);
  ...
  const handleEditClick = (record: VehicleStateHistoryRecord) => {
  ...
  const handleEditConfirm = async () => {
    ...
    setUpdating(true);
    try {
      await VehicleStateHistoryService.updateVehicleStateHistoryReason(
        recordToEdit.id,
        editedReason.trim()
      );
          ...
          {!loading && !error && history.length === 0 && (
            <Box sx={{ textAlign: 'center', py: 4 }}>
              <Typography color="text.secondary">
                No hay registros de cambios de estado para este vehículo
              </Typography>
            </Box>
          )}
                            ...
                            <>
                              ...
                              <Tooltip 
                                title={
                                  canDelete 
                                    ? "Eliminar registro" 
                                    : "No se puede eliminar el registro más reciente (estado actual del vehículo)"
                                }
                              >
                                <span>
                                  <IconButton
