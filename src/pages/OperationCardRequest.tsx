...
interface OperationCardRequestProps {
  hideTitle?: boolean;
}

export default function OperationCardRequest({ hideTitle = false }: OperationCardRequestProps): JSX.Element {
  ...
  const onGenerate = async () => {
    ...
    try {
      const res = await api.post(
        `/documents/operation-card-request/${selectedVehicle.id}`,
        {},
        { responseType: 'blob' as any }
      );
      
      const dispo = (res as any)?.headers?.['content-disposition'] || (res as any)?.headers?.['Content-Disposition'];
      let filename = 'solicitud_tarjeta_operacion.pdf';
      ...
      success('Solicitud generada exitosamente');
    } catch (e: any) {
      const msg = e?.response?.data?.message || 'No se pudo generar la solicitud';
        ...
        <>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 0.5 }}>
            ...
            <Typography variant="h5" sx={{ fontWeight: 700, letterSpacing: 0.3 }}>
              Solicitud de Tarjeta de Operación
            </Typography>
          </Box>
